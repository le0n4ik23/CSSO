...
public:
	union
	{
		...
		DEFINE_MEMBER_N(int, m_nNextThinkTick, 0x90);
		...
		DEFINE_MEMBER_N(float, m_hTonemapController, 0x2EBC);
	    ...
	    DEFINE_MEMBER_N(bool, m_fogenable, 0x638);
		DEFINE_MEMBER_N(float, m_fogstart, 0x618);
		DEFINE_MEMBER_N(float, m_fogend, 0x61c);
		DEFINE_MEMBER_N(float, m_fogmaxdensity, 0x624);
		DEFINE_MEMBER_N(int, m_fogcolorPrimary, 0x608);
		DEFINE_MEMBER_N(int, m_fogcolorSecondary, 0x60c);
		DEFINE_MEMBER_N(int, m_vecMinsPreScaled, 0x8);
		DEFINE_MEMBER_N(int, m_vecMaxsPreScaled, 0x14);
		DEFINE_MEMBER_N(const char,  m_szLastPlaceName[50], 0x3180);
		DEFINE_MEMBER_N(float, FOV, 0x2ed4);

		//someabsurdshit
		DEFINE_MEMBER_N(Vector3, m_nRelativeDirectionOfLastInjury, 0x2be4);
