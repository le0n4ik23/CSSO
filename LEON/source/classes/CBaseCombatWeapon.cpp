...
bool CBaseCombatWeapon::IsCanShoot()
{
    static decltype(this) weapon = nullptr;
   
    static auto stored_tick = 0;
    if (weapon != this || stored_tick >= globals::localPlayer->m_nTickBase) {
        weapon = this;
        stored_tick = globals::localPlayer->m_nTickBase;
        return false; //cannot shoot first tick after switch
    }

    if (this->m_iClip1 <= 0 || !globals::localPlayer)
        return false;

    auto flServerTime = globals::localPlayer->m_nTickBase * globalVars->intervalPerTick;

    return globals::localPlayer->m_flNextAttack <= flServerTime;
}
float CBaseCombatWeapon::GetWeaponPenetration()
{
    CEntity* weapon = (CEntity*)this;
    if (!weapon)
        return 0.5f; // Вернуть значение по умолчанию, если оружие не определено

    int weaponID = weapon->GetClientClass()->classID;

    switch (weaponID)
    {
        // Определите коэффициенты проникновения для разных типов оружия
    case ClassID::CWeaponAWP:
        return 0.99f; // Пример для снайперской винтовки AWP
        break;

    case ClassID::CWeaponAK47:
    case ClassID::CWeaponM4A4:
    case ClassID::CWeaponM4A1:
        return 0.97f; // Пример для автоматической винтовки AK47, M4A4, M4A1
        break;

        // Добавьте другие кейсы для остальных типов оружия

    default:
        return 0.5f; // Значение по умолчанию, если тип оружия не определен
    }
}
